...
function getDefaultSimulatorTarget() {
  return require('./list-emulator-build-targets')
    .run()
...
module.exports.run = function(buildOpts) {
  ...
  return require('./list-devices')
        ...
        return promise.then(function(theTarget) {
          if (!theTarget) {
            return getDefaultSimulatorTarget().then(function(defaultTarget) {
              ...
              events.emit(
                'log',
                `Building for "${emulatorTarget}" Simulator (${defaultTarget.identifier}, ${
                  defaultTarget.simIdentifier
                }).`
              );
            ...
            events.emit(
              'log',
              `Building for "${emulatorTarget}" Simulator (${theTarget.identifier}, ${theTarget.simIdentifier}).`
            );
            return emulatorTarget;
          }
        });
    ...
    .then(function() {
      ...
      var xcodebuildArgs = getXcodeBuildArgs(
        projectName,
        projectPath,
        configuration,
        buildOpts.device,
    ...
    .then(function() {
      ...
      function packageArchive() {
        var xcodearchiveArgs = getXcodeArchiveArgs(
          projectName,
          projectPath,
          buildOutputDir,
          exportOptionsPath,
        ...
        return superspawn.spawn('xcodebuild', xcodearchiveArgs, {
          cwd: projectPath,
          printCommand: true,
          stdio: 'inherit'
        });
...
function getXcodeBuildArgs(
  ...
  if (buildFlags) {
    if (typeof buildFlags === 'string' || buildFlags instanceof String) {
      ...
      // buildFlags is an Array of strings
  ...
  if (isDevice) {
    options = [
      '-workspace',
      customArgs.workspace || projectName + '.xcworkspace',
      '-scheme',
      customArgs.scheme || projectName,
      '-configuration',
      customArgs.configuration || configuration,
      '-destination',
      customArgs.destination || 'generic/platform=iOS',
      '-archivePath',
      customArgs.archivePath || projectName + '.xcarchive'
    ];
    ...
    options = [
      '-workspace',
      customArgs.project || projectName + '.xcworkspace',
      '-scheme',
      customArgs.scheme || projectName,
      '-configuration',
      customArgs.configuration || configuration,
      '-sdk',
      customArgs.sdk || 'iphonesimulator',
      '-destination',
      customArgs.destination || 'platform=iOS Simulator,name=' + emulatorTarget
    ];
    buildActions = ['build'];
    settings = [
  ...
  xcodebuildArgs = options
    .concat(buildActions)
    .concat(settings)
    .concat(customArgs.otherFlags);
...
module.exports.help = function help() {
  ...
  console.log(
    '    build --codeSignIdentity="iPhone Distribution" --provisioningProfile="926c2bd6-8de9-4c2f-8407-1016d2d12954"'
  );
  // TODO: add support for building different archs

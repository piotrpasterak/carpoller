...
var keep_these_frameworks = ['MobileCoreServices.framework', 'CoreGraphics.framework', 'AssetsLibrary.framework'];
...
var handlers = {
    ...
    install: function(obj, plugin, project, options) {
      ...
      if (!fs.existsSync(srcFile)) {
        throw new CordovaError(
          'Cannot find resource file "' + srcFile + '" for plugin ' + plugin.id + ' in iOS platform'
        );
      }
      if (fs.existsSync(destFile)) {
        throw new CordovaError(
          'File already exists at destination "' +
            destFile +
            '" for resource file specified by plugin ' +
            plugin.id +
            ' in iOS platform'
        );
      }
  ...
  framework: {
    ...
    install: function(obj, plugin, project, options) {
      ...
      if (!custom) {
        ...
        if (keepFrameworks.indexOf(src) < 0) {
          if (obj.type === 'podspec') {
            ...
            events.emit(
              'verbose',
              util.format('Adding non-custom framework to project... %s -> %s', src, JSON.stringify(opt))
            );
            project.xcode.addFramework(src, opt);
            events.emit(
              'verbose',
              util.format('Non-custom framework added to project. %s -> %s', src, JSON.stringify(opt))
            );
          }
        }
        return;
      ...
      if (!fs.existsSync(srcFile))
        throw new CordovaError('Cannot find framework "' + srcFile + '" for plugin ' + plugin.id + ' in iOS platform');
      if (fs.existsSync(targetDir))
        throw new CordovaError(
          'Framework "' + targetDir + '" for plugin ' + plugin.id + ' already exists in iOS platform'
        );

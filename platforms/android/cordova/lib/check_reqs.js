...
module.exports.check_java = function() {
  ...
  return Q()
    ...
    .then(function() {
      return Q.denodeify(child_process.exec)('javac -version').then(
        outputs => {
    ...
    if (!hasAndroidHome) {
        ...
        if (path.basename(parentDir) === 'tools' || fs.existsSync(path.join(grandParentDir, 'tools', 'android'))) {
          ...
          throw new CordovaError(
            "Failed to find 'ANDROID_HOME' environment variable. Try setting it manually.\n" +
              "Detected 'android' command at " +
              parentDir +
              " but no 'tools' directory found near.\n" +
      ...
      if (avdmanagerInPath) {
        ...
        if (path.basename(parentDir) === 'bin' && path.basename(grandParentDir) === 'tools') {
          ...
          throw new CordovaError(
            "Failed to find 'ANDROID_HOME' environment variable. Try setting it manually.\n" +
              "Detected 'avdmanager' command at " +
              parentDir +
              " but no 'tools" +
              path.sep +
              "bin' directory found near.\n" +
              'Try reinstall Android SDK or update your PATH to include valid path to SDK' +
              path.sep +
              'tools' +
              path.sep +
...
module.exports.check_android_target = function(originalError) {
  ...
  return android_sdk.list_targets().then(function(targets) {
    ...
    var msg =
      'Please install Android target / API level: "' +
      desired_api_level +
      '".\n\n' +
...
module.exports.check_all = function() {
  ...
  var checkFns = [this.check_java, this.check_android, this.check_android_target, this.check_gradle];
  ...
  return checkFns
    .reduce(function(promise, checkFn, idx) {
      ...
      return promise.then(checkFn).then(
        function(version) {

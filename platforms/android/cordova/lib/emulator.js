  ...
  return Promise.resolve()
    ...
    .then(function(emulatorId) {
      return self.get_available_port().then(function(port) {
        ...
        child_process.spawn('emulator', args, { stdio: 'inherit', detached: true, cwd: emulator_dir }).unref();
    ...
    .then(function(emulatorId) {
      if (!emulatorId) {
        return Promise.reject(new CordovaError('Failed to start emulator'));
      }
...
module.exports.wait_for_emulator = function(port) {
  ...
  return Promise.resolve().then(function() {
    ...
    return Adb.shell(emulator_id, 'getprop dev.bootcomplete').then(
      function(output) {
  ...
  return Adb.shell(emulator_id, 'ps').then(function(output) {
    if (output.match(/android\.process\.acore/)) {
      ...
      return new Promise(resolve => {
        ...
        setTimeout(() => {
          const updated_time =
            time_remaining >= 0 ? Math.max(time_remaining - CHECK_BOOTED_INTERVAL, 0) : time_remaining;
...
module.exports.create_image = function(name, target) {
  ...
  if (target) {
    return superspawn
      .spawn('android', ['create', 'avd', '--name', name, '--target', target])
    ...
    console.log(
      'WARNING : Project target not found, creating avd with a different target but the project may fail to install.'
    );
    // TODO: there's a more robust method for finding targets in android_sdk.js
    return superspawn.spawn('android', ['create', 'avd', '--name', name, '--target', this.list_targets()[0]]).then(
      function() {
...
module.exports.resolveTarget = function(target) {
  return this.list_started().then(function(emulator_list) {
    if (emulator_list.length < 1) {
      return Promise.reject(
        new CordovaError('No running Android emulators found, please start an emulator before deploying your project.')
      );
    }
      ...
      return Promise.resolve().then(function() {
        ...
        function installPromise() {
          return adbInstallWithOptions(target.target, apk_path, execOptions).catch(function(error) {
            ...
            events.emit(
              'warn',
              'Uninstalling app from device and reinstalling it because the ' +

...
function getInstallDestination(obj) {
  ...
  if (appReg.test(obj.targetDir)) {
    ...
    if (obj.src.endsWith('.java')) {
      return path.join(APP_MAIN_PREFIX, 'java', obj.targetDir.replace(srcReg, ''), path.basename(obj.src));
    } else if (obj.src.endsWith('.aidl')) {
      return path.join(APP_MAIN_PREFIX, 'aidl', obj.targetDir.replace(srcReg, ''), path.basename(obj.src));
    } else if (libsReg.test(obj.targetDir)) {
      if (obj.src.endsWith('.so')) {
        return path.join(APP_MAIN_PREFIX, 'jniLibs', obj.targetDir.replace(libsReg, ''), path.basename(obj.src));
      } else {

...
function updateConfigFilesFrom(sourceConfig, configMunger, locations) {
  ...
  xmlHelpers.mergeXml(sourceConfig.doc.getroot(), config.doc.getroot(), 'android', /* clobber= */ true);
...
function updateProjectAccordingTo(platformConfig, locations) {
  ...
  if (java_files.length === 0) {
    ...
    events.emit(
      'log',
      'Multiple candidate Java files that extend CordovaActivity found. Guessing at the first one, ' + java_files[0]
    );
  }
...
function updateIcons(cordovaProject, platformResourcesDir) {
  ...
  icons.forEach((icon, key) => {
    ...
    if (icon.foreground) {
      ...
      if (
        !icon.src &&
        (icon.foreground.startsWith('@color') || path.extname(path.basename(icon.foreground)) === '.xml')
      ) {

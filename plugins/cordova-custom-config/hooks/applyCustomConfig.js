...
// Npm dependencies
var logger, fs, _, et, plist, xcode, tostr, os, fileUtils;
...
var applyCustomConfig = (function() {
      ...
      if (item.type === 'preference') {
        ...
        if (multiple) {
          var uniqueSelector =
            ...
            multiple.uniqueBy +
            "='" +
            data.attrib['android:' + multiple.uniqueBy] +
            "']";
  ...
  function updateXCBuildConfiguration(item, buildConfig, mode) {
    ...
    for (var blockName in buildConfig) {
      ...
      if (
        ...
        var value =
          item.quote && (item.quote === 'none' || item.quote === 'key') ? item.value : quoteEscape(item.value);
    ...
    _.each(configItems, function(item) {
      ...
      if (item.name) {
        ...
        if (itemBuildType === fileBuildType) {
          ...
          if (item.name.match('#INCLUDE') && !fileContents.match(value)) {
            ...
            if (fileContents.match(escapedName) && item.xcconfigEnforce !== 'false') {
              doReplace();
            } // presence of item is being enforced, so add it to the relevant .xcconfig
        ...
        // so if buildType="debug", want to overrwrite in build.xcconfig
          ...
          item.name.match('CODE_SIGN_IDENTITY') &&
          itemBuildType === 'debug' &&
          fileBuildType === 'none' &&
          !item.xcconfigEnforce
        ) {
          doReplace();
        }
      }
  ...
  function deployAssetCatalog(targetName, targetDirPath, configItems) {
    ...
    _.each(configItems, function(item) {
      ...
      if (!item.scale || !item.scale.match(/^[1-3]{1}x$/)) {
        logger.error(
          'scale must be specified as 1x, 2x or 3x for ' +
            srcImgFileName +
            ' in ' +
            targetName +
            ' asset catalog - skipping image'
        );
        return;
      }
      var entry = {
  ...
  applyCustomConfig.init = function(ctx) {
    ...
    if (context.hook !== runHook) {
      logger.debug(
        "Aborting applyCustomConfig.js because current hook '" +
          context.hook +
          "' is not configured hook '" +
          runHook +
          "'"
      );
      return complete();
...
module.exports = function(ctx) {
  try {
    ...
    e.message =
      TAG +
      ': Error loading dependencies for ' +
      SCRIPT_NAME +
      " - ensure the plugin has been installed via cordova-fetch or run 'npm install cordova-custom-config': " +
      e.message;
